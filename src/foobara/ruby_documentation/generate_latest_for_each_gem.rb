  ...
  module RubyDocumentation
    class GenerateLatestForEachGem < Foobara::Command
      ...
      def execute
        ...
        each_project do
          each_version do
            if latest_build_version_for_its_minor_version?
              install_gem
      ...
      def latest_build_version_for_its_minor_version?
        project.latest_build_versions.include?(version)
      end
      ...
      def remove_timestamps
        Dir.chdir expanded_output_dir do
          Bundler.with_unbundled_env do
            cmd = "find . -name '*.html' -exec sed -E -i '/" \
                  'Generated on \w+ \w+ [0-9: ]+by/{N;N;s/.*\n(.*)\n.*$/Generated by\1/}\' {} +'
